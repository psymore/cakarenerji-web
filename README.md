...
- Themes: `<html data-theme>` (`light` = the `:root` palette in `app/globals.css`, `dark` = token overrides in `app/themes.css`). `lib/theme.ts` holds the theme list, the pre-paint inline script (saved choice, else OS preference) and the helpers; `components/ThemeToggle.tsx` is the header button. Colours are tokens with roles (`--panel` dark band, `--strong` panel colour as text/border on the page surface, `--on-sun`, `--glass`…), so a new theme is one token block in `themes.css` (plus `/ui-2` overrides in `app/ui-2/css/tokens.css`). Do not hard-code colours in component CSS.
