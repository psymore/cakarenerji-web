...
- The project lives at `D:\CodeSpace\cakarenerji-web` and has its own repo (branch `main`, remote `origin` = github.com/psymore/cakarenerji-web, which deploys through GitHub Actions on every push to `main`). Run git from there.
