...
(async () => {
  ...
  const browser = await chromium.launch({ executablePath: CHROME, headless: !headed, args: headed ? [] : ["--enable-gpu-rasterization"] });
  const rows = [];
  for (const throttle of [1, 4]) {
    for (const [name, css] of Object.entries(variants)) {
      rows.push(await run(browser, name, css, throttle, 1440, 900));
    }
  }
  await browser.close();
  console.table(rows);
