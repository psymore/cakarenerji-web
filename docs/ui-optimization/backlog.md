...
|---|---|---|---|---|---|---|---|
| UIO-01 | Hero light drops to about 48-50 fps without GPU raster (old PCs, VMs, remote desktop, hardware acceleration off). Steady 60 fps with the light removed. | `measurements.md`, software-raster rows | Done in part: the lit grid window is gone, only the 840 px glow remains. A transform cannot remove CPU compositing cost. | Yes: visual (look of the light) | U-19 | P2 | decided: grid removed, glow eased in one rAF loop and driven by touchmove on touch (built 2026-09-21); software-raster fps not re-measured |
...
| UIO-04 | Mobile hero: the upper half was empty (the grid, now removed, showed only at the bottom). The photo now fills it. | `ux/m-home` screenshot | Lower mobile `min-height`, or a taller, brighter grid on narrow screens. | Yes: design | U-21 | P2 | open |
