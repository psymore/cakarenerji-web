...
- **Design:** PV-module idea. Panel blue, silver busbar lines, one gold sun. Archivo (width axis) and Source Serif 4. The home hero is the live photo with a warm sun glow that rises once, then glides to the pointer or finger (no cell grid any more).
- **Typo gallery `/duzeltmeler`:** 18 fixes shown as before/after with a faded red border on the live wording, plus 7 errors left as they are. Opened from the header settings menu, `noindex`, built from the `{ t, fixed }` records in the content files.
...
- **UI/UX left (backlog `docs/ui-optimization/backlog.md`, UIO-01…12):** empty upper half of the mobile hero, mobile cookie notice size and its effect on mobile LCP (4.4 s), blog post body heading levels, software-raster jank of the hero light. The hero light now glides smoothly on touch (see UIO-01). See `docs/ui-optimization/ui-ux/audit-2026-09-21.md` and `animation-performance/measurements.md`.
