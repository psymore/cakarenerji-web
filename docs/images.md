...
## Known limits
- **Home hero.** The live first image (sunset over hills with a panel array) is `isteam/getty/2155735205`, a stock image hosted outside the site's own folder; `lib/images.ts` builds its URL (values starting with `getty/`). The green field (`pexels-tom-fisk`, id `homeField`) is the second hero photo on the home page (behind the contact button and phone number, cropped, under a panel veil; branch `feature/hero-split-photos`). Found from the owner's page dump (U-25, done).
