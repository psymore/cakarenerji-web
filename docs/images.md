...
## Known limits
- **Home hero.** The live first image (sunset over hills with a panel array) is `isteam/getty/2155735205`, a stock image hosted outside the site's own folder; `lib/images.ts` builds its URL (values starting with `getty/`). The green field (`pexels-tom-fisk`, id `homeField`) is the second strip under the hero on the home page (cropped, with a shadowed divider; on branch `feature/hero-two-photos` until it is merged). Found from the owner's page dump (U-25, done).
