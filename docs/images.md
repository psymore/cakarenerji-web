...
## Known limits
- **Home hero.** The live first image (sunset over hills with a panel array) is `isteam/getty/2155735205`, a stock image hosted outside the site's own folder; `lib/images.ts` builds its URL (values starting with `getty/`). The green field (`pexels-tom-fisk`, id `homeField`) is the second hero photo and, depending on the home layout picked in the settings menu, a strip under the hero or under "Hakkımızda" or the background of the "Çakar Enerji A.Ş." block (`lib/home-layout.ts`). Found from the owner's page dump (U-25, done).
