...
| header settings menu | Design name | The second design is now called "İkincil tasarım (yuvarlatılmış kenarlar)" in the "Tasarım" section instead of "İkinci (UI 2)" (the route stays `/ui-2`). Older rows that say "UI 2" or "İkinci tasarım (UI 2)" describe the old label | Requested. |
